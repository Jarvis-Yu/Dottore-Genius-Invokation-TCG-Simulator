    ...
    def test_behaviour(self):
        ...
        game_state = step_action(game_state, Pid.P1, CardAction(
            ...
            instruction=StaticTargetInstruction(
                dices=ActualDices({Element.ANEMO: 1, Element.DENDRO: 1}),
        ...
        game_state = step_action(game_state, Pid.P1, CardAction(
            card=SendOff,
            instruction=StaticTargetInstruction(
                dices=ActualDices({Element.HYDRO: 1, Element.ELECTRO: 1}),
        ...
        game_state = step_action(game_state, Pid.P1, CardAction(
            card=SendOff,
            instruction=StaticTargetInstruction(
                dices=ActualDices({Element.GEO: 1, Element.PYRO: 1}),
        ...
        game_state = step_action(game_state, Pid.P1, CardAction(
            card=SendOff,
            instruction=StaticTargetInstruction(
                dices=ActualDices({Element.CRYO: 2}),
                target=StaticTarget.from_summon(Pid.P2, SesshouSakuraSummon),
            ),
        ))
        p2_summons = game_state.get_player2().get_summons()
