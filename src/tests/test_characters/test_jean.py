...
class TestJean(unittest.TestCase):
    ...
    assert type(BASE_GAME.get_player1().just_get_active_character()) is Jean
    ...
    def test_elemental_skill1(self):
        ...
        self.assertIn(Element.PYRO, p2c2.get_elemental_aura())
        self.assertIn(Element.PYRO, p2c3.get_elemental_aura())
    ...
    def test_elemental_burst(self):
        ...
        self.assertFalse(p2ac.get_elemental_aura().has_aura())
        self.assertEqual(p1cs.just_get_character(1).get_hp(), 8)
        self.assertEqual(p1cs.just_get_character(2).get_hp(), 9)
        self.assertEqual(p1cs.just_get_character(3).get_hp(), 10)
        self.assertIn(DandelionFieldSummon, p1.get_summons())
        self.assertEqual(p1.get_summons().just_find(DandelionFieldSummon).usages, 2)
    ...
    def test_dandelion_field_summon(self):
        ...
        self.assertEqual(p2c3.get_hp(), 9)
        self.assertFalse(p2c1.get_elemental_aura().has_aura())
        self.assertIn(Element.ELECTRO, p2c2.get_elemental_aura())
        self.assertIn(Element.ELECTRO, p2c3.get_elemental_aura())
        self.assertEqual(p1ac.get_hp(), 8)
        self.assertIn(DandelionFieldSummon, p1.get_summons())
        self.assertEqual(p1.get_summons().just_find(DandelionFieldSummon).usages, 1)

        game_state = next_round(game_state)
        p1 = game_state.get_player1()
        self.assertNotIn(DandelionFieldSummon, p1.get_summons())
