...
class TestRana(unittest.TestCase):
    def test_behaviour(self):
        ...
        next_char_elem = game_state.get_player1().get_characters(
        ).get_nth_next_alive_character_in_activity_order(1).ELEMENT()
        prev_dice_num = game_state.get_player1().get_dice()[next_char_elem]

        game_state = replace_character(game_state, Pid.P1, Albedo, 1)

        # Non elemental skill cannot trigger
        game_state = step_skill(game_state, Pid.P1, CharacterSkill.SKILL1)
        curr_dice_num = game_state.get_player1().get_dice()[next_char_elem]
